...
if ($('#items').val() == "") { // condition qui me permet de faire apparaitre mon boutton jouer
    $('.button').html('<button type="button" class="btn btn-info" name="button" id="buttonPlay"> Jouer</button>');
}
$(function () {
    ...
    // je crée un tableau contenant tout les tableaux créé au dessus, et un second tableau contenant les seconde version des tableaux
    ...
    $('#buttonPlay').click(function () { // fonction au clic de #buttonPlay
        ...
        $('.button').html('<button type="button" class="btn btn-warning" name="button" id="buttonDown"> Rejouer</button>');
        $(function () {
        ...
        if (itemNumber == 0) { // régle si 1 objet
            ...
            if (select == 0) { // si le select thème = 0 c'est les fruits
                var x = 1;
                while (x < 11) { // boucle while qui fait apparaitre 10 fruits de chaque comme clonne de l'objet invisible # draggable 1. .draggable({ revert: "invalid" }) permet de faire revenir les objets si ils ne sont pas draggable
                    $('#draggable1').clone().prop('id', ).prop('class', 'pomme fruits').prop('alt', 'une jolie pomme').attr('src', 'assets/imgGames/pomme.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'orange').prop('class', 'orange fruits').prop('alt', 'une jolie orange').attr('src', 'assets/imgGames/orange.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'banane').prop('class', 'banane fruits').prop('alt', 'une jolie banane').attr('src', 'assets/imgGames/banane.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'poire').prop('class', 'poire fruits').prop('alt', 'une jolie poire').attr('src', 'assets/imgGames/poire.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'ananas').prop('class', 'ananas fruits').prop('alt', 'un joli ananas').attr('src', 'assets/imgGames/ananas.png').prependTo("#panier").draggable({ revert: "invalid" });
                    x++;
                }
                // fait apparaite l'image de la table dans la div # table
                $('#table').append("<img src='assets/imgGames/table.png' alt='La table ou l'on pose les fruits' />")
            } else if (select == 1) { // si le select thème = 1 les annimaux de la ferme..... même chose qu'au dessus
                var x = 1;
                while (x < 11) {
                    $('#draggable1').clone().prop('id', 'vache').prop('class', 'vache animals').prop('alt', 'une jolie vache').attr('src', 'assets/imgGames/vache.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'mouton').prop('class', 'mouton animals').prop('alt', 'un joli mouton').attr('src', 'assets/imgGames/mouton.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'cochon').prop('class', 'cochon animals').prop('alt', 'un petit cochon').attr('src', 'assets/imgGames/cochon.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'poule').prop('class', 'poule animals').prop('alt', 'une petite poule').attr('src', 'assets/imgGames/poule.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'cheval').prop('class', 'cheval animals').prop('alt', 'un joli cheval').attr('src', 'assets/imgGames/cheval.png').prependTo("#panier").draggable({ revert: "invalid" });
                    x++;
                }
                $('#table').append("<img src='assets/imgGames/prairie.jpg' alt='La pature ou se reposent les animaux' />")
            } else if (select == 2) {
                var x = 1;
                while (x < 11) {
                    $('#draggable1').clone().prop('id', 'culotte').prop('class', 'culotte clothing').prop('alt', 'une petite culotte').attr('src', 'assets/imgGames/culotte.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 't-shirt').prop('class', 't-shirt clothing').prop('alt', 'un joli t-shirt').attr('src', 'assets/imgGames/tshirt.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'pull').prop('class', 'pull clothing').prop('alt', 'un pull de sport').attr('src', 'assets/imgGames/pull.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'chaussette').prop('class', 'chaussette clothing').prop('alt', 'une petite chaussette').attr('src', 'assets/imgGames/chaussette.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'pantalon').prop('class', 'pantalon clothing').prop('alt', 'un pantalon môche').attr('src', 'assets/imgGames/pantalon.png').prependTo("#panier").draggable({ revert: "invalid" });
                    x++;
                }
                $('#table').append("<img src='assets/imgGames/valise.png' alt='une valise pour partir en vacances' />")
            } else if (select == 3) {
                var x = 1;
                while (x < 11) {
                    $('#draggable1').clone().prop('id', 'yahourt').prop('class', 'yahourt fridge').prop('alt', 'un yahourt à l\'Oréo').attr('src', 'assets/imgGames/yahourt.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'beurre').prop('class', 'beurre fridge').prop('alt', 'un pot de beurre').attr('src', 'assets/imgGames/beurre.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'lait').prop('class', 'lait fridge').prop('alt', 'un verre de lait').attr('src', 'assets/imgGames/lait.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'jus d\'oranges').prop('class', 'jus d\'oranges fridge').prop('alt', 'une verre de jus d\'oranges').attr('src', 'assets/imgGames/jus d\'orange.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'tomate').prop('class', 'tomate fridge').prop('alt', 'une belle tomate').attr('src', 'assets/imgGames/tomate.png').prependTo("#panier").draggable({ revert: "invalid" });
                    x++;
                }
                $('#table').append("<img src='assets/imgGames/fridge.png' alt='un frigidaire bien rempli' />")
            } else if (select == 4) {
                var x = 1;
                while (x < 11) {
                    $('#draggable1').clone().prop('id', 'crabe').prop('class', 'crabe seaAnimals').prop('alt', 'un yahourt à l\'Oréo').attr('src', 'assets/imgGames/crabe.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'poisson').prop('class', 'poisson seaAnimals').prop('alt', 'un pot de beurre').attr('src', 'assets/imgGames/poisson.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'pieuvre').prop('class', 'pieuvre seaAnimals').prop('alt', 'un verre de lait').attr('src', 'assets/imgGames/pieuvre.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'tortue').prop('class', 'tortue seaAnimals').prop('alt', 'une verre de jus d\'oranges').attr('src', 'assets/imgGames/tortue.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'étoile de mer').prop('class', 'étoile de mer seaAnimals').prop('alt', 'une belle tomate').attr('src', 'assets/imgGames/étoile de mer.png').prependTo("#panier").draggable({ revert: "invalid" });
                    x++;
                }
            ...
            if (select == 0) {
                var x = 1;
                while (x < 11) {
                    $('#draggable1').clone().prop('id', 'pomme').prop('class', 'pomme fruits').prop('alt', 'une jolie pomme').attr('src', 'assets/imgGames/pomme.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'orange').prop('class', 'orange fruits').prop('alt', 'une jolie orange').attr('src', 'assets/imgGames/orange.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'banane').prop('class', 'banane fruits').prop('alt', 'une jolie banane').attr('src', 'assets/imgGames/banane.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'poire').prop('class', 'poire fruits').prop('alt', 'une jolie poire').attr('src', 'assets/imgGames/poire.png').prependTo("#panier").draggable({ revert: "invalid" });
                    $('#draggable1').clone().prop('id', 'ananas').prop('class', 'ananas fruits').prop('alt', 'un joli ananas').attr('src', 'assets/imgGames/ananas.png').prependTo("#panier").draggable({ revert: "invalid" });
        ...
        $(".table").droppable({ // permet de rendre la class table droppable, c'est a dire receptive aux objets draggable
            accept(draggable) {
                ...
                if (itemNumber == 0) { // si le nombre d'objet est 1
                    ...
                    const validFruits = [selectedFirstIvtemAlea, selectedSecondItemAlea]; // les deux tiems sélectionnés seront valide
                    let isValid = false;
                    validFruits.forEach((c) => (isValid = draggable.hasClass(c) ? true : isValid));
                    return isValid;
                }
            ...
            drop: function (event, ui) { // On entre dans l'event drop
                ...
                if (currentId === selectedFirstIvtemAlea) { // on controle si l'id de le premier objet correspond bien a ce qui est attendu
                    ...
                    if (count < number) { // si le nombre de premier objet posé est inférieur au nombre requis (number) il ne se passe rien, l'objet retourne à se position d'origine si il ne correspond pas
                        ...
                        $("#gameRules").append("<h2 class='alert'><center>Tu as posé trop " + (selectedFirstIvtemAlea == 'ananas' || selectedFirstIvtemAlea == 'orange' ? 'd\'' : 'de ') + (selectedFirstIvtemAlea == 'cheval' ? 'chevaux' : (selectedFirstIvtemAlea == 'étoile de mer' ? 'étoiles de mer' : selectedFirstIvtemAlea)) + (selectedFirstIvtemAlea == 'ananas' || selectedFirstIvtemAlea == 'jus d\'oranges' || selectedFirstIvtemAlea == 'étoile de mer' ? '' : 's ') + "</center></h2><audio src='assets/audio/alert.ogg' autoplay></audio>");
                        ...
                        if (itemNumber == 1 && $('.victoryOne').html() == selectedFirstIvtemAlea + number && $('.victoryTwo').html() == selectedSecondItemAlea + secondNumber) {
                            $("#gameRules").append("<h2 class='win'><center>C'est gagné !  <img class='speaker' src='assets/imgGames/speaker.png' alt='Un speaker qui parle'></center></h2>");
                            $('.speaker').click(function () {
                ...
                // pour les reste les commentaire du dessus s'appliquent également   
                        ...
                        $("#gameRules").append("<h2 class='alert'><center>Tu as posé trop " + (selectedSecondItemAlea == 'ananas' || selectedSecondItemAlea == 'orange' ? 'd\'' : 'de ') + selectedSecondItemAlea + (selectedSecondItemAlea == 'ananas' ? '' : 's ') + "</center></h2><audio src='assets/audio/alert.ogg' autoplay></audio>");
                    } else if (secondCount = secondNumber) {
                        ...
                        if (itemNumber == 1 && $('.victoryOne').html() == selectedFirstIvtemAlea + number && $('.victoryTwo').html() == selectedSecondItemAlea + secondNumber) {
                            $("#gameRules").append("<h2 class='bigWin'><center>C'est gagné !  <img class='speaker' src='assets/imgGames/speaker.png' alt='Un speaker qui parle'></center></h2>");
                            $('.speaker').click(function () {
                                setTimeout(function () { $('#gameRules').append('<audio src="assets/audio/gagne.mp3" autoplay></audio>'); }, 500);
                            });
                            victoryCount++;
